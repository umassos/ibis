...
def send_request(request, stub, service_name, node_name, input_rate, result_collector: List, request_name):
    ...
    result = {"service_name": service_name,
              ...
              "total_response_time": (response.front_end_time - response.front_start_time) * 1000,
              ...
              "GPU_execution_time": response.gpu_process_time}
...
def simulate_requests(service_name: str, node_name: str, ip_address: str, input_rate: float,
    ...
    with grpc.insecure_channel(ip_address) as channel:
        ...
        for i in range(iterations):
            ...
            request = frontend_service_pb2.InferenceRequest(image_name=image[1], image_type=str(t),
                                                                    image=image[0])
...
def main():
    ...
    parser.add_argument("-t", "--time", default=10, type=int, dest="time", help="Duration of the experiment")
    ...
    with open("results/current_running_config", 'r') as f:
        f.read()


    workload_basename = "gpu_workload_"
    with open("results/current_running_config", 'r') as f:
        workload_basename+=f.read()
